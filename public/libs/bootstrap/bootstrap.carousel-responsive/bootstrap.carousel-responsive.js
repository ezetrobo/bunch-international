(Sawubona.carousel = function(){
	...
	for(var i = 0; i < this.carousels.length; i++){
		...
		var setItems = (self.attr('data-items') != undefined && self.attr('data-items') != '') ? self.attr('data-items').split(',') : [1, 2, 3, 4, 5];
		//	Gardo sus items en una variable global
		window["bkp-items-" + i] = (window["bkp-items-" + i] != undefined) ? window["bkp-items-" + i] : self.children('.carousel-inner').children('.carousel-item').removeClass('active').removeClass('carousel-item');
		var items = window["bkp-items-" + i];
		var width = window.innerWidth;
		//	Segun el ancho de la pantalla es la cantidad de items-responsive que se mostraran
		var cantidad =(width >= 1024) ? (setItems[4] != undefined ? setItems[4] : 5) : (width >= 991) ? (setItems[3] != undefined ? setItems[3] : 4) : ((width >= 576) ? (setItems[2] != undefined ? setItems[2] : 3) : ((width >= 481) ? (setItems[1] != undefined ? setItems[1] : 2) : (setItems[0] != undefined ? setItems[0] : 1) ));
