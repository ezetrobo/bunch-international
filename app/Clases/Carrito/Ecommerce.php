...
use App\Clases\Carrito\CuentaGateway;
use App\Clases\Carrito\Ecommerce;
use App\Clases\Carrito\MedioPago;
use App\Clases\Carrito\eCommerceTipoEnvio;
...
class Ecommerce {
	...
	public function getConfiguracion() {
		try
		{
			if (is_string(config('main.ID_ECOMMERCE_ENCODE'))) {
				$xUrl = config('main.URL_REST')
				. "ECommerce/ObtenerConfiguracion/".config('main.ID_ECOMMERCE_ENCODE');
				$vEcommerce = $this->getEcommerce($xUrl);
	...
	public function setConfiguracion($xData){
				...
				if ($xDataTipoEnvio->costoEnvio->costosEnviosZonas != NULL) {
					...
					foreach ($xDataTipoEnvio->costoEnvio->costosEnviosZonas as $xDataCostoEnvio) {
						if ($tmp == $xDataCostoEnvio->zona) {
							...
							$xCostoEnvio = new CostoEnvioZona($xDataCostoEnvio->idCostoEnvioZona,$xDataCostoEnvio->zona,$xDataCostoEnvio->valor,$xDataCostoEnvio->hastaKG,$xDataCostoEnvio->excedente,NULL);
	...
	public function getTipoEnvio() {
		return $this->eCommerceTipoEnvio;
	}
	...
	private function getEcommerce($xUrl){
		try {
			if (is_string($xUrl)) {
				$vDataAPI = Sawubona::getCache($xUrl, config('main.CACHE_ECOMMERCE'));

				return $this->setConfiguracion($vDataAPI);
			}
		} catch (Exception $e) {
			Log::error($e);
		}
