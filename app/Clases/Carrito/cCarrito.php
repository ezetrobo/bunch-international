...
class cCarrito {
	...
	public function __construct() {
		...
		$this->entrega = null;
		$this->paso = 0;
		$this->banderas = array(false, false, false, false, false);		
	...
	public function deleteProducto($xIndice = null) {
		try {
			unset(session('carrito')->productos[$xIndice]);
		} catch (Exception $e) {
			Log::error($e);
		}
	}
	...
	public function setPaso($xPaso = 0) {
	...
	/*	Funcion para pago por mercadopago */
	public function setMp($oCuentaGatewayPago) {
		...
		$preference_data =
			...
			"back_urls" =>
			[
				"success" => $_SERVER['HTTP_HOST'] . "/Mp/compraok", 
			...
			"notification_url" => (($_SERVER['SERVER_ADDR'] == '10.1.2.10') ? $_SERVER['HTTP_HOST'] : "http://sawubona.com.ar") . "/Mp/notificacion",
		...
		$urlWebApi = config('main.URL_WEBAPI'). "/MP/Agregar/" . $oCuentaGatewayPago->merchantId . "/" . $oCuentaGatewayPago->merchantKey;
